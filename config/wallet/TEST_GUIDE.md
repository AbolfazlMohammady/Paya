...
```env
BASE_URL=http://localhost:8000             # آدرس بازگشت؛ برای تست عمومی از ngrok یا دامنه واقعی استفاده کنید
PAYMENT_GATEWAY_DEFAULT=sepehr
SEPEHR_ENABLED=True
SEPEHR_TERMINAL_ID=98743989               # مقدار نمونه، در محیط واقعی مقدار اصلی را قرار دهید
...
# برای تست بدون اتصال به درگاه، حالت Mock را فعال کنید (فقط در توسعه)
SEPEHR_MOCK_MODE=True
```

> در محیط production مقدار `SEPEHR_MOCK_MODE` باید حذف یا برابر False باشد تا درخواست‌ها به درگاه واقعی ارسال شود.
...
### مرحله 2: بررسی وضعیت کیف پول

> از نسخه فعلی به بعد کیف پول هر کاربر در لحظه ساخت User به صورت خودکار (signal) ایجاد می‌شود. کافی است وضعیت آن را بررسی کنید.
...
اگر پاسخ 404 دریافت کردید، یعنی signal اجرا نشده یا کاربر بدون کیف پول ایجاد شده است؛ در این صورت می‌توانید همچنان از endpoint ایجاد کیف پول استفاده کنید.

### مرحله 3: درخواست شارژ (Mock یا واقعی)
...
### مرحله 4: هدایت کاربر به صفحه پرداخت (فقط در صورت اتصال واقعی)

در حالت Mock نیازی به باز کردن `payment_url` نیست. برای اتصال واقعی، کاربر باید به صفحه پرداخت درگاه (سپهر) هدایت شود.

### مرحله 5: شبیه‌سازی Callback در حالت Mock

در حالت Mock یا زمانی که درگاه به callback شما دسترسی ندارد، پاسخ را به صورت دستی POST کنید:
...
{
  "respcode": "0",
  "invoiceid": "req_abc123def456",          # همان request_id دریافتی از مرحله قبل
  "digitalreceipt": "MOCK_RECEIPT_1",       # شناسه یکتا (دلخواه)
  "Authority": "MOCK_TOKEN_..."             # همان authority دریافتی از مرحله قبل
}
```

> در اتصال واقعی، مقادیر `invoiceid` و `digitalreceipt` باید مطابق داده‌های ارسالی سپهر باشد. در حالت Mock، مقادیر ساختگی کافی است.
...
**پاسخ نمونه (پس از callback موفق):**
...
- اگر حالت Mock فعال است ولی هنوز به درگاه واقعی وصل می‌شوید، مقدار `SEPEHR_MOCK_MODE=True` در `.env` را بررسی و سرویس را ری‌استارت کنید. در پاسخ‌های Mock مقدار `mock_mode: true` در `debug_info` قابل مشاهده است.
...
# در حالت Mock، callback را به صورت دستی صدا می‌زنیم
requests.post(
...
- [ ] کیف پول ایجاد شده
- [ ] درخواست شارژ موفق است (در حالت Mock مقدار `mock_mode` را بررسی کنید)
- [ ] payment_url و authority دریافت شده‌اند
