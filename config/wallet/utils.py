...
def validate_iranian_iban(iban):
    """
    اعتبارسنجی شماره شبا (IBAN) ایران
    فرمت: IR + 2 رقم checksum + 3 رقم کد بانک + 18 رقم شماره حساب = 26 رقم
    """
    if not iban:
    ...
    # بررسی طول (باید 26 رقم باشد)
    if len(iban) != 26:
        return False, "IBAN must be exactly 26 characters"
    
    # بررسی شروع با IR
    if not iban.startswith('IR'):
        return False, "IBAN must start with 'IR'"
    
    # بررسی اینکه بقیه کاراکترها عدد باشند
    if not iban[2:].isdigit():
        return False, "IBAN must contain only digits after 'IR'"
    
    # اعتبارسنجی checksum (الگوریتم MOD-97-10)
    # تبدیل IR به اعداد: I=18, R=27
    rearranged = iban[4:] + '1827' + iban[2:4]
...
def transfer_to_iban(sender_wallet, iban, amount, description='', metadata=None):
    """
    انتقال وجه به شماره شبا (انتقال واقعی و فوری)
    
    این یک انتقال واقعی است که:
    - موجودی فوراً از کیف پول کم می‌شود
    - تراکنش با status 'completed' ثبت می‌شود
    - وجه به حساب بانکی خارج از شبکه واریز می‌شود (ممکن است نیاز به پردازش بانکی داشته باشد)
    
    نکته: واریز به بانک ممکن است زمان ببرد، اما موجودی فوراً کم می‌شود
    """
    # قفل کردن کیف پول
    if not acquire_wallet_lock(sender_wallet.id):
    ...
    try:
        ...
        if not is_valid:
            raise ValueError(f"Invalid IBAN: {error_msg}")
        ...
        transfer_metadata.setdefault('iban', iban)
        transfer_metadata.setdefault('direction', 'outgoing')
        transfer_metadata.setdefault('transfer_type', 'external_bank')
        
        # ایجاد تراکنش با status 'completed'
        # موجودی فوراً کم می‌شود
        transaction = Transaction.objects.create(
            ...
            balance_after=sender_balance_after,  # موجودی کم شده
            description=description or f'انتقال به شبا {iban[-4:]}',
            status='completed',  # انتقال فوری انجام شده
            transfer_method='iban',
            metadata=transfer_metadata
        )
        
        # به‌روزرسانی موجودی (کم کردن فوری)
        ...
        release_wallet_lock(sender_wallet.id)
